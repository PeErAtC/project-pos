...
export default function SalesPage() {
    ...
    const [isCategoryOpen, setIsCategoryOpen] = useState(false);  // สถานะเปิด/ปิด dropdown หมวดหมู่
...
// ฟังก์ชัน fetchCategories
    ...
    const handleCategorySelect = (categoryId) => {
        ...
        setSelectedCategoryId(categoryId);  // อัปเดต selectedCategoryId ด้วยค่าที่เลือก
    };
    // เมื่อผู้ใช้เลือกหมวดหมู่
    const handleCategorySelects = (categoryId) => {
        setSelectedCategoryId(categoryId);
        setIsCategoryOpen(false); // ปิด dropdown หลังเลือกหมวดหมู่
    };
   ...
   // เปลี่ยนชื่อ remainingDue เพื่อลดการประกาศซ้ำ
    ...
    const filteredProductss = products.filter((product) => {
        const matchesSearchQuery = product.p_name && product.p_name.toLowerCase().includes(searchQuery.toLowerCase());
        const matchesCategory = selectedCategoryId ? product.category_id === parseInt(selectedCategoryId) : true;
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{
                       ...
                       {/* เลือกหมวดหมู่และช่องค้นหาสินค้าในบรรทัดเดียวกัน */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <select
                                ...
                                style={{
                                    ...
                                    borderRadius: '5px',
                                    flex: 1,  // ทำให้ select ขยายตัวให้พอดีกับพื้นที่
                                }}
                            >
                                <option value="">เลือกหมวดหมู่</option>
                                {categories.map((category) => (
                                    <option key={category.id} value={category.id}>
                                        {category.c_name}  {/* ใช้ c_name แทน name */}
                                    </option>
                                ))}
                            </select>
                            ...
                            <input
                                ...
                                style={{
                                    ...
                                    border: '1px solid #ccc',
                                    borderRadius: '5px',
                                    flex: 2,  // ทำให้ช่องค้นหาขยายใหญ่กว่าหมวดหมู่
                                }}
                            />
                        </div>
