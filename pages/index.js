...
export default function HomePage() {
  ...
  const handleIconClick = (page) => {
    if (!isLoggedIn) {
      ...
      router.push('/login');
    } else {import Image from 'next/image';
import { useState, useEffect } from 'react';
import { useRouter } from 'next/router';
...
const styles = {
  ...
  iconContainer: {
    ...
    marginBottom: '20px',
    padding: '100px',
    backgroundColor: '#ffffff', // พื้นหลังสีขาวสำหรับกล่องทั้งหมด
    borderRadius: '20px',       // โค้งมนรอบกรอบใหญ่
    boxShadow: '0px 8px 20px rgba(0, 0, 0, 0.15)', // เพิ่มเงาให้กรอบใหญ่
  },
  iconBox: {
    display: 'flex',
